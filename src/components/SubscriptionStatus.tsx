...
export default function SubscriptionCenterPage({
  ...
  const [localPlan, setLocalPlan] = useState<PlanKey | null>(currentPlan ?? null);
  const [pendingChange, setPendingChange] = useState<
    { plan: PlanKey; type: "upgrade" | "downgrade" | "purchase"; message?: string } | null
  >(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError(err?.message || err?.detail || "Failed to fetch subscription status");
      } finally {
  ...
  const createCheckoutFor = async (plan: PlanKey) => {
    ...
    setLoadingPlan(plan);
    try {
      const path = `/stripe/create-checkout-session?plan=${encodeURIComponent(plan)}&mode=subscription`;
      ...
      if (resp.url) {
        ...
        if (resp.id && typeof onCreated === "function") {
          try { onCreated(resp.id, plan); } catch {}
        }
      ...
      if (resp.downgrade) {
        ...
        if (resp.stripe_subscription) {
          alert(resp.message ?? "Downgrade requested — pending webhook confirmation.");
      ...
      if (resp.upgrade) {
        if (resp.stripe_subscription) {
          alert(resp.message ?? "Upgrade requested — will finalize after payment.");
      ...
      alert("Failed to start/change subscription: " + (err?.message || String(err)));
    } finally {
      setLoadingPlan(null);
    }
  };
  ...
  const openBillingPortal = async () => {
    ...
    try {
      const resp = await jwtRequest("/stripe/create-portal-session", "POST", null);
      if (resp?.url) {
        window.location.href = resp.url;
        return;
      }
  ...
  const currentRank = hasActiveSubscription ? PLAN_RANK[localPlan as PlanKey] : -1;

  return (
    <main className={`min-h-screen bg-slate-50 py-12 ${className}`}>
      <div className="max-w-7xl mx-auto px-6">
        <header className="text-center mb-4">
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900">Choose the right plan for your dog</h1>
          <p className="mt-4 text-slate-600 max-w-3xl mx-auto">Bigger plans with more support. Pick per-dog subscription that grows with your pet. Secure payments via Stripe.</p>
          ...
          {pendingChange ? (
            <div className="mt-4 inline-flex items-center gap-3 rounded-full bg-amber-50 px-5 py-3 shadow">
              <div className="text-sm text-amber-800 font-semibold">
                {pendingChange.type === "upgrade" ? "Upgrade pending" : pendingChange.type === "downgrade" ? "Downgrade pending" : "Change pending"}
              </div>
              <div className="text-xs text-amber-700">•</div>
              <div className="text-xs text-amber-700">{pendingChange.message ?? `Switching to ${PLANS[pendingChange.plan].title} — will finalize after Stripe confirms payment.`}</div>
            </div>
          ) : (
            <div className="mt-4 inline-flex items-center gap-3 rounded-full bg-white px-5 py-3 shadow">
              <div className="text-sm text-slate-700 font-semibold">{status ? status.toUpperCase() : "NO SUBSCRIPTION"}</div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          {(Object.keys(PLANS) as PlanKey[]).map((key) => {
            ...
            let btnClass = "flex-1 px-6 py-4 rounded-2xl text-sm font-semibold shadow-md text-white bg-brand-charcoal";

            if (isCurrent) {
              buttonText = "Current plan";
              btnDisabled = true;
              btnClass = "flex-1 px-6 py-4 rounded-2xl text-sm font-semibold shadow-md text-brand-midgrey bg-brand-offwhite cursor-not-allowed";
            } else if (isLower) {
              buttonText = `Downgrade to ${plan.title}`;
              buttonHandler = () => openDowngradeConfirm(key);
              btnClass = "flex-1 px-6 py-4 rounded-2xl text-sm font-semibold shadow-md text-white bg-rose-600";
            } else if (isHigher) {
              buttonText = `Upgrade to ${plan.title}`;
              buttonHandler = () => createCheckoutFor(key);
              btnClass = isPopular(key) ? "flex-1 px-6 py-4 rounded-2xl text-sm font-semibold shadow-md text-white bg-gradient-to-r from-indigo-600 to-indigo-500" : "flex-1 px-6 py-4 rounded-2xl text-sm font-semibold shadow-md text-white bg-brand-charcoal";
            } else {
            ...
            return (
              <article key={key} className={`relative bg-white rounded-3xl p-8 md:p-10 shadow-xl flex flex-col justify-between ${plan.accent ?? ""}`} aria-label={`${plan.title} plan`}>
                ...
                <div>
                  ...
                  <div className="mt-6 flex items-end gap-4">
                    <div className="text-5xl md:text-6xl font-extrabold text-slate-900">{plan.priceLabel}</div>
                  ...
                  <div className="mt-6 text-sm text-slate-500">Need help? Reply to the email on your account.</div>
                </div>
        ...
        <section className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-2xl p-8 shadow">
            ...
            <p className="mt-3 text-sm text-slate-600">All subscriptions are billed monthly per dog. Cancel anytime from the billing portal. Taxes and local fees may apply.</p>
            <div className="mt-4 text-sm text-slate-500">{error ? <span className="text-rose-600">{error}</span> : null}</div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-8 shadow">
            ...
            <dl className="mt-4 space-y-4 text-sm text-slate-600">
              <div>
                ...
                <dd className="mt-1">Yes — upgrade or downgrade anytime. Upgrades take effect immediately in the billing portal.</dd>
              </div>
              <div>
        ...
        <footer className="mt-12 text-center text-sm text-slate-500">Need a custom plan or team discount? Contact support via the email on your account.</footer>
      </div>
      ...
      {confirmOpen && modalPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-w-xl w-full bg-white rounded-xl p-6 shadow-lg">
            <h3 className="text-xl font-semibold">Confirm downgrade to {PLANS[modalPlan].title}</h3>
            <p className="mt-2 text-sm text-slate-600">Downgrading may remove features from your current plan. You'll be moved to the {PLANS[modalPlan].title} plan.</p>
            ...
            <div className="mt-4">
              ...
              <ul className="mt-2 list-disc ml-5 text-sm text-slate-700">
                {modalLoses.length ? (
                  ...
                  <li>No major features will be removed — just some perks may be unavailable.</li>
                )}
              </ul>
            </div>

            <div className="mt-6 flex gap-3 justify-end">
              <button onClick={() => setConfirmOpen(false)} className="px-4 py-2 rounded-md border text-sm text-slate-700 bg-white">Cancel</button>
              <button onClick={confirmDowngrade} className="px-4 py-2 rounded-md bg-rose-600 text-white text-sm">Yes, downgrade</button>
            </div>
          </div>
        </div>
      )}
...
function isPopular(k: PlanKey) { return k === "therapeutic"; }
