...
const SubmissionsList: React.FC = () => {
  const { submissions, bulkApproveSubmissions } = useAdmin();
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'under_review': return <FileText className="h-4 w-4 text-blue-500" />;
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Case Submissions</h1>
            <p className="text-lg text-gray-600">Review and manage diagnosis submissions</p>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0 lg:space-x-4">
            ...
            <div className="flex space-x-4">
              ...
              <select
                ...
                onChange={(e) => setSearchParams({ ...Object.fromEntries(searchParams), priority: e.target.value })}
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              >
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredSubmissions.map((submission) => (
                  <tr key={submission.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{submission.dogData.breed}</div>
                          {submission.isReevaluation && (
                            <div className="text-xs text-blue-600 font-medium">Re-evaluation</div>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getUrgencyColor(submission.aiDiagnosis.urgencyLevel)}`}>
                        {submission.aiDiagnosis.urgencyLevel}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                          <div 
                            ...
                            style={{ width: `${submission.aiDiagnosis.confidence * 100}%` }}
                          ></div>
                        </div>
